...
public class MicrowavePlugin implements Plugin<Project> {
    public static final String NAME = "microwave";

    @Override
    public void apply(final Project project) {
        project.getExtensions().create(NAME, MicrowaveExtension.class);

        project.afterEvaluate(actionProject -> {
            final MicrowaveExtension extension = MicrowaveExtension.class.cast(actionProject.getExtensions().findByName(NAME));
        ...
        project.task(new HashMap<String, Object>() {{
            put("type", MicrowaveTask.class);
            put("group", "Embedded Application Server");
            put("description", "Starts a microwave!");
        }}, NAME);
    }
}
